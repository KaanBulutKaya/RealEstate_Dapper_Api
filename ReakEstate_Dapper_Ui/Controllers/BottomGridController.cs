...
using ReakEstate_Dapper_Ui.Dtos.BottomGridDtos;
using System.Text;
...
namespace ReakEstate_Dapper_Ui.Controllers
{
    public class BottomGridController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var jsondate = JsonConvert.SerializeObject(createBottomGridDto);
            StringContent stringContent = new StringContent(jsondate, Encoding.UTF8, "application/json");
            var responseMessage = await client.PostAsync("https://localhost:44350/api/BottomGrids", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var reponseMessage = await client.DeleteAsync($"https://localhost:44350/api/BottomGrids/{id}");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:44350/api/BottomGrids/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateBottomGrid(UpdateBottomGridDto updateBottomGridDto)
        {
            ...
            var responseMessage = await client.PutAsync("https://localhost:44350/api/BottomGrids/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
